	...
	for _, rm := range wr.req.ResourceMetrics {
		...
		for _, sc := range rm.ScopeMetrics {
			for _, metricData := range sc.Metrics {
				...
				case *opentelemetrypb.Metric_Gauge:
					for _, p := range metric.Gauge.DataPoints {
						ts := tsFromNumericPoint(metricData.Name, p, wr.baseLabels)
						wr.tss = append(wr.tss, ts)
					}

				case *opentelemetrypb.Metric_Sum:
					if metric.Sum.AggregationTemporality != opentelemetrypb.AggregationTemporality_AGGREGATION_TEMPORALITY_CUMULATIVE {
				...
				case *opentelemetrypb.Metric_Histogram:
					if metric.Histogram.AggregationTemporality != opentelemetrypb.AggregationTemporality_AGGREGATION_TEMPORALITY_CUMULATIVE {
				...
				case *opentelemetrypb.Metric_Summary:
...
func tsFromNumericPoint(metricName string, point *opentelemetrypb.NumberDataPoint, baseLabels []prompb.Label) prompb.TimeSeries {
	var value float64
	switch v := point.Value.(type) {
	case *opentelemetrypb.NumberDataPoint_AsInt:
		value = float64(v.AsInt)
	case *opentelemetrypb.NumberDataPoint_AsDouble:
		value = v.AsDouble
	}
...
func tssFromSummaryPoint(metricName string, point *opentelemetrypb.SummaryDataPoint, baseLabels []prompb.Label) []prompb.TimeSeries {
...
func tssFromHistogramPoint(metricName string, point *opentelemetrypb.HistogramDataPoint, baseLabels []prompb.Label) []prompb.TimeSeries {
	var tss []prompb.TimeSeries
	...
	switch d := point.Sum_.(type) {
	case *opentelemetrypb.HistogramDataPoint_Sum:
	default:
		logger.Warnf("opentelemetry histogram bad sum type, want: HistogramDataPoint_Sum, got: %v", d)
		return nil
	}
	sumTs := newPromPBTs(metricName+"_sum", t, point.Sum_.(*opentelemetrypb.HistogramDataPoint_Sum).Sum, isStale, pointLabels...)
	countTs := newPromPBTs(metricName+"_count", t, float64(point.Count), isStale, pointLabels...)
...
type writeContext struct {
	...
	req        opentelemetrypb.ExportMetricsServiceRequest
...
func (wr *writeContext) unpackFrom(r io.Reader, isJSON bool) error {
	...
	parseFunc := wr.req.Unmarshal
	if isJSON {
		parseFunc = wr.req.UnmarshalJSONExportMetricsServiceRequest
	}
